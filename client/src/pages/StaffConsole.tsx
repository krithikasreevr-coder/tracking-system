...
import { BarChart3, BookOpen, Check, ChevronRight, ClipboardList, Edit3, GraduationCap, LayoutDashboard, LogOut, Plus, Trash2, UserPlus, Users, X } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { toast } from "sonner";

type StaffUser = { id: number; name: string; email: string | null; role: "staff" | "student"; createdAt: Date; updatedAt: Date };
type AssignmentDraft = { subject: string; title: string; description: string; dueDate: string; targetType: "class" | "students"; classId: string; studentIds: number[] };
const blankDraft = (): AssignmentDraft => ({ subject: "", title: "", description: "", dueDate: "", targetType: "class", classId: "", studentIds: [] });
...
export default function StaffConsole({ user }: { user: StaffUser }) {
  ...
  function openEdit(assignment: typeof assignments[number]) {
    ...
    setDraft({ subject: assignment.subject, title: assignment.title, description: assignment.description ?? "", dueDate: dateInputValue(new Date(assignment.dueDate)), targetType: assignment.classId ? "class" : "students", classId: assignment.classId ? String(assignment.classId) : "", studentIds: assignment.targetStudents.map(student => student.id) });
    setShowComposer(true); setActivePanel("assignments");
  }
  async function saveAssignment(event: React.FormEvent) {
    event.preventDefault();
    const payload = { subject: draft.subject, title: draft.title, description: draft.description || null, dueDate: new Date(`${draft.dueDate}T12:00:00`).getTime(), classId: draft.targetType === "class" && draft.classId ? Number(draft.classId) : null, studentIds: draft.targetType === "students" ? draft.studentIds : [] };
    try {
  ...
  return (
    <div className="console-shell">
      ...
      <main className="console-main">
        <header className="workspace-header"><div><p className="eyebrow">STAFF COMMAND CONSOLE</p><h1>{activePanel === "overview" ? "Signal overview" : activePanel === "assignments" ? "Assignment control" : "Class directory"}</h1></div><button className="primary-action compact" onClick={openCreate}><Plus size={16} /> New assignment</button></header>
